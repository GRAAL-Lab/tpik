...
tpik::PriorityLevel::PriorityLevel(std::string ID) :
		taskNumber_(0), Ae_(0) {
	ID_ = ID;
}
...
std::string tpik::PriorityLevel::GetID() const
...
void tpik::PriorityLevel::UpdateJacobian() {
	...
	for (auto& task : std::vector<std::shared_ptr<Task>>(level_.begin() + 1,
			level_.end())) {
...
void tpik::PriorityLevel::UpdateInternalActivationFunction() {
	...
	for (auto& task : std::vector<std::shared_ptr<Task>>(level_.begin() + 1,
		...
		Eigen::MatrixXd Anew = rml::RightJuxtapose(Ai_,
				Eigen::MatrixXd::Zero(Ai_.rows(), ANewTask.cols()));
		Ai_ = rml::UnderJuxtapose(Anew,
				rml::RightJuxtapose(
						Eigen::MatrixXd::Zero(ANewTask.rows(), Ai_.cols()),
						ANewTask));
