...
PriorityLevel::PriorityLevel(const std::string ID)
    : ID_(ID), actionTransitionA_(0), taskNumber_(0), priorityLevelSpace_(0) {}
  ...
  for (auto &task : level_) {
    if (isAiInitialized) {
      ...
      if (task->GetIsActive()) {
        ...
        ANewTask =
            Eigen::MatrixXd::Zero(task->GetTaskSpace(), task->GetTaskSpace());
      }
...
Eigen::MatrixXd PriorityLevel::GetActivationFunction() {
  ...
  for (auto &task : level_) {
    ...
    Aextern_.block(priorityLevelSpace_ - lastTaskSpace,
                   priorityLevelSpace_ - lastTaskSpace, taskSpacei,
                   taskSpacei) = AexternalBloc;
