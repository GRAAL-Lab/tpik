...
const BellShapedParameter& InequalityTask::GetIncreasingBellShapedParameter() { return increasingBellShape_; }

const BellShapedParameter& InequalityTask::GetDecreasingBellShapedParameter() { return decreasingBellShape_; }
...
void InequalityTask::CheckInitialization() throw(ExceptionWithHow)
{
    if (!initializedTaskParameter_) {
        ...
        std::string how
            = "[InequalityTask] Not initialized taskParameter struct, use SetTaskParameter() for task " + ID_;
        notInitializedTaskParameter.SetHow(how);
        throw(notInitializedTaskParameter);
    }
    if (!initializedIncreasingBellShapeParameter_
        && (inequalityType_ == InequalityTaskType::Increasing || inequalityType_ == InequalityTaskType::Inbetween)) {
        NotInitialziedTaskParameterException notInitializedBellShapeIncreasing;
        std::string how = "[InequalityTask] Not initialized incresingBellShape struct, use "
    ...
    if (increasingBellShape_.xmax.size() != taskSpace_
        ...
        std::string how = "[InequalityTask] Wrong size incresingBellShape struct, task space = "
    ...
    if (!initializedDecreasingBellShapeParameter_
        && (inequalityType_ == InequalityTaskType::Decreasing || inequalityType_ == InequalityTaskType::Inbetween)) {
        NotInitialziedTaskParameterException notInitializedBellShapeDecreasing;
        std::string how = "[InequalityTask] Not initialized decreasingBellShape struct, use "
                          "SetDecreasingBellShapedParameter() for task "
            + ID_;
        notInitializedBellShapeDecreasing.SetHow(how);
        throw(notInitializedBellShapeDecreasing);
    }

    if (decreasingBellShape_.xmax.size() != taskSpace_
        && (inequalityType_ == InequalityTaskType::Decreasing || inequalityType_ == InequalityTaskType::Inbetween)) {
        NotInitialziedTaskParameterException wrongBellShapeDecreasingSize;
        std::string how = "[InequalityTask] Wrong size decreasingBellShape struct, task space = "
            + std::to_string(taskSpace_) + " use SetDecreasingBellShapedParameter() for task " + ID_;
...
void InequalityTask::SaturateReference() { rml::SaturateVector(taskSpace_, taskParameter_.saturation, x_dot_); }
