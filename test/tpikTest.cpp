...
//    y = solver->ComputeVelocities();
   ...
   //AeRows.setOnes(testTask1->TaskSpace());
   //AeRows = AeRows * 5;
   //Eigen::VectorXd AeRows2;
   //AeRows2.setOnes(testTask2->TaskSpace());
   //AeRows2 = AeRows2 * 2;
   //std::cout << "AeRows: " << AeRows.transpose() << std::endl;
   //std::cout << "AeRows2: " << AeRows2.transpose() << std::endl;
   //Eigen::MatrixXd A = actionManager->GetPriorityLevel(IDPL1)->ActivationFunction();
   //std::cout << "A before settin AeRows =\n " << A << std::endl;
   //testTask1->ExternalActivationFunction(AeRows);
   //testTask2->ExternalActivationFunction(AeRows2);
   //A = actionManager->GetPriorityLevel(IDPL1)->ActivationFunction();
